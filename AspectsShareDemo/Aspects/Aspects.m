...
// Tracker 和 Class 是一对一的关系，且唯一对应
@interface AspectTracker : NSObject
- (id)initWithTrackedClass:(Class)trackedClass;
@property (nonatomic, strong) Class trackedClass;
@property (nonatomic, readonly) NSString *trackedClassName;
@property (nonatomic, strong) NSMutableSet *selectorNames; // 只有本类hook的方法，才会存在这里面
@property (nonatomic, strong) NSMutableDictionary *selectorNamesToSubclassTrackers; // selectorName: [subclassTracter...]
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    static NSSet *disallowedSelectorList; // 方法黑名单
    static dispatch_once_t pred;
    dispatch_once(&pred, ^{
    ...
    if (class_isMetaClass(object_getClass(self))) {
        ...
        /*
         ...
         NSMutableDictionary <Class, AspectTracker> * swizzledClassesDict
         */
        NSMutableDictionary *swizzledClassesDict = aspect_getSwizzledClassesDict(); // 单利字典
        ...
        do {
            ...
            if ([tracker.selectorNames containsObject:selectorName]) {
                if (klass == currentClass) {
                    // 第一次循环，才会进入这里
                    // Already modified and topmost! 本类 已经hook了，依然可以添加hook
                    return YES;
                }
                NSString *errorDescription = [NSString stringWithFormat:@"Error: %@ already hooked in %@. A method can only be hooked once per class hierarchy.", selectorName, NSStringFromClass(currentClass)];
                AspectError(AspectErrorSelectorAlreadyHookedInClassHierarchy, errorDescription);
                return NO; // 父类hook过这个方法了，不允许再hook了
            }
        } while ((currentClass = class_getSuperclass(currentClass)));

        // Add the selector as being modified.
        currentClass = klass; // 重置
...
// 递归调用，获取tracker下，hook了selector的子类集合，其实只有一个
- (NSSet *)subclassTrackersHookingSelectorName:(NSString *)selectorName {
